...
from .utils import convert_id_list_to_obj, find_genes_in_collection, transform_tpm_data, update_user_gene_request, get_or_create_user_tier_and_request
...
def gene_autocomplete(request):
    ...
    if query:
        ...
        genes = Gene.objects.filter(
            gene_name__icontains=query
        ) | Gene.objects.filter(
            ensembl_id__icontains=query
        )

        # Limit the results to 10 matches
        results = [{"id": gene.ensembl_id.split(".")[0] + "_" + gene.gene_name, "label": f"{gene.ensembl_id} - {gene.gene_name}"} for gene in genes[:10]]
    else:
        results = []
...
def bulk_rna_analysis_list(request):
    ...
    user = request.user
    user_tier, user_request, usage_percentage = get_or_create_user_tier_and_request(user)
...
def explore_analysis(request, analysis_id):
    ...
    gene_collections = GeneCollection.objects.filter(
        ...
        (Q(linked_analyses=selected_dataset) & Q(private_collection=False) & Q(customer_visible=True))
    ).distinct()
    ...
    if request.method == "POST":
        ...
        if selection_type == "individual":
            ...
            selected_collection = get_object_or_404(GeneCollection, id=selected_collection_id)
            selected_genes = [gene.df_string for gene in selected_collection.included_genes.all()]
        ...
        if user_tier.tier.name == "Free":
            ...
            accessible_genes, non_accessible_genes = find_genes_in_collection(
                ...
                free_gene_collection
            )
        elif user_tier.tier.name == "Premium":
            premium_access_collection = GeneCollection.objects.get(collection_name="Premium access")
            accessible_genes, non_accessible_genes = find_genes_in_collection(
                selected_gene_objects,
                premium_access_collection
            )
        ...
        if accessible_genes and selected_conditions:
            if len(accessible_genes) == 1:
                ...
                if group_by_condition == "day":
                    data_sorted = sorted(selected_conditions_for_plot, key=lambda x: x.split("_")[1])  # Sort by the 2nd element
                ...
                plot_data = processed_tsv_df.loc[accessible_genes[0].df_string, selected_conditions_for_plot].to_dict()
                ...
                plot_data = processed_tsv_df.loc[gene_df_ids, selected_conditions_for_plot].values.tolist()
                print(plot_data)
                plot_type = 'heatmap'

    # Render the template with gene, gene set, and condition options
    return render(request, 'explore_analysis.html', {
        'user_tier': user_tier,
        'user_request': user_request,
        'usage_percentage': usage_percentage,
...
def pca_view(request, analysis_id, plot_3d=True):
    ...
    #path_to_tsv = "/Users/jambler/Downloads/EXP24003358_MJ_Vijay_adipocytes_TPM.tsv"
    #tsv_df = pd.read_csv(path_to_tsv, sep='\t', index_col=0)
    ...
    log_tpm_df = log_tpm_df.loc[log_tpm_df.var(axis=1) > 0.1]  # Optional low-variance filtering
    ...
    groups = [condition.split('_')[0] for condition in conditions]  # Get the first part of the condition string

    if plot_3d:
        print("plot_3d")
        # Group the PCA results by group
        grouped_pca_data = defaultdict(lambda: {'x': [], 'y': [], 'z': [], 'labels': []})
        ...
        grouped_pca_data_list = [(group, data) for group, data in grouped_pca_data.items()]

        # Pass the PCA data and groups to the template
        return render(request, 'explore_analysis_pca_3d.html', {
            'analysis': analysis,
        ...
        group_numeric = label_encoder.fit_transform(groups)  # Convert group names to numeric labels

        # Pass the PCA data, group labels, and groups to the template
        return render(request, 'explore_analysis_pca.html', {
            'analysis': analysis,
            'pc1_values': pc1_values,
            'pc2_values': pc2_values,
            'conditions': conditions,
            'group_numeric': group_numeric.tolist(),  # Pass numeric labels for coloring
            'groups': groups,  # Pass the original group names for hover text
        })
    """
...
def gene_collection_list(request, analysis_id=0):
    ...
    return render(request, 'gene_collection_list.html', {
        'table': table,
...
def view_gene_collection(request, collection_id):
    ...
    RequestConfig(request, paginate={"per_page": 10}).configure(table)  # Enable pagination and searching

    return render(request, 'view_gene_collection.html', {
...
def create_gene_collection(request, analysis_id):
    ...
    return render(request, 'create_gene_collection.html', {
        'form': form,
...
def edit_gene_collection(request, collection_id):
    collection = get_object_or_404(GeneCollection, id=collection_id, created_by=request.user)
    ...
    return render(request, 'edit_gene_collection.html', {
        'form': form,
        'collection': collection,
    })
...
def delete_gene_collection(request, collection_id):
    ...
    return render(request, 'delete_gene_collection.html', {
        'collection': collection,
    })
...
def load_genes_from_gtf(request):
    ...
    if load_from_s3:
        ...
        try:
            ...
            return JsonResponse({"error": f"Failed to load GTF file from S3: {str(e)}"}, status=500)
    else:
    ...
    try:
        with gzip.open(gtf_file_path, 'rt') as gtf_file:
            for line in gtf_file:
                ...
                if columns[2] == 'gene':
                    ...
                    long_name = attribute_dict.get('gene_long_name', None)  # Not all GTFs have this field
        ...
        return JsonResponse({"error": f"Failed to process GTF file: {str(e)}"}, status=500)
...
def view_user_genes(request):
    ...
    return render(request, "view_user_genes.html", {
